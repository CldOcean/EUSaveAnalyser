/**
 * Loader for EU4 localisation files.
 *
 * Two quirks make this non-trivial:
 *
 * 1. Mods write non-Latin text with the same "letter stream" encoding as save
 *    files (see `packages/eu4-parser/src/encoding.ts`). The files are valid
 *    UTF-8, but each letter was produced by encoding a *byte* — and bytes
 *    0x80..0x9F were round-tripped through CP1252, so `‘` (U+2018) has to be
 *    turned back into byte 0x91 before decoding.
 * 2. Later files override earlier ones, and a mod may only ship a few files,
 *    leaving the rest to the base game. Callers pass files in load order.
 */
...
export function decodeLocalisationValue(value: string): string {
  const bytes: number[] = [];
...
/** Load `key -> text` from one `.yml` file. Missing files yield an empty map. */
export function loadLocalisationFile(path: string): Map<string, string> {
  ...
  for (const line of text.split(/\r?\n/)) {
    ...
    if (!match) continue;
    out.set(match[1] as string, decodeLocalisationValue(match[2] as string));
  }
  return out;
}

/** Load several files in order; later entries override earlier ones. */
export function loadLocalisation(paths: readonly string[]): Map<string, string> {
  const out = new Map<string, string>();
  for (const path of paths) {
    for (const [key, value] of loadLocalisationFile(path)) out.set(key, value);
  }
  return out;
}
 ...
 * `nahuatl`, ... as raw keys.
 *
 * Every name below was read out of a localisation file on this machine with
 * `loadLocalisationFile`, never invented:
 ...
 */
export const RELIGION_FALLBACK: ReadonlyMap<string, string> = new Map<string, string>([
...
/**
 * Best-effort localisation lookup.
 *
 * EU4 appends suffixes to keys (`_ADJ`, `_name`); some keys are bare constants
 * already (`orthodox`, `russian`). Try the key, then its common variants, then the
 * religion fallback table, then the raw key so the UI degrades to something
 * readable. A localisation entry always beats the fallback.
 */
export function localise(table: Map<string, string>, key: string | undefined): string {
  if (!key) return '';
  return (
    table.get(key) ??
    table.get(`${key}_name`) ??
    table.get(`${key.toUpperCase()}`) ??
